...
static void
unlock_dialog_keyfiles_set_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    adw_action_row_set_subtitle (ADW_ACTION_ROW (self->keyfile_row), new_label ? new_label->str : _("None Selected"));
}
...
on_keyfile_chooser_clicked_cb (GduUnlockDialog *self)
{
    ...
    gtk_file_dialog_open (file_dialog, GTK_WINDOW (self->parent_window), NULL, unlock_dialog_keyfiles_set_cb, self);
}

static void
unlock_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    UDisksEncrypted *encrypted = UDISKS_ENCRYPTED (source_object);
    GduUnlockDialog *self = user_data;
    g_autoptr(GError) error = NULL;

    if (!udisks_encrypted_call_unlock_finish (encrypted, NULL, /* out_cleartext_device */
                                              result, &error))
...
do_unlock (GduUnlockDialog *self)
{
    ...
    passphrase = gtk_editable_get_text (GTK_EDITABLE (self->passphrase_entry));
    udisks_encrypted_call_unlock (self->udisks_encrypted, passphrase, g_variant_builder_end (&options_builder),
                                  NULL, /* cancellable */
                                  unlock_cb, self);
}
...
void
gdu_unlock_dialog_show (GtkWindow *parent_window, UDisksObject *object)
{
    ...
    g_signal_connect_swapped (self->key_files_store, "notify::n-items",
                              G_CALLBACK (unlock_dialog_update_unlock_button_cb), self);

    secret_password_lookup (&luks_passphrase_schema, NULL, /* GCancellable */
                            luks_find_passphrase_cb, self, "gvfs-luks-uuid",
                            udisks_block_get_id_uuid (self->udisks_block), NULL); /* sentinel */
}
