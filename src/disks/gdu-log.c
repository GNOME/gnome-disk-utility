...
matches_domain (const char *log_domains, const char *domain)
{
    ...
    if (!log_domains || !*log_domains || !domain || !*domain)
        return FALSE;
...
should_log (const char *log_domain, GLogLevelFlags log_level)
{
    ...
    if (verbosity < 4 && g_strcmp0 (log_domain, "GdkPixbuf") == 0 && (!domains || !strstr (domains, log_domain)))
        return FALSE;
               ...
               gsize n_fields, gpointer user_data)
{
    ...
    if (ostream) {
        ...
        if (stderr_is_journal && g_log_writer_journald (log_level, fields, n_fields, user_data) == G_LOG_WRITER_HANDLED)
    ...
    /* Add local time */
    {
        ...
        g_string_append_printf (log_str, "%s.%04d ", buffer, (int) ((now % G_USEC_PER_SEC) / 100));
    }
    ...
    if (fatal_criticals && (log_level & G_LOG_LEVEL_CRITICAL))
        log_level |= G_LOG_FLAG_FATAL;
    else if (fatal_warnings && (log_level & (G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING)))
        log_level |= G_LOG_FLAG_FATAL;
...
gdu_log_handler (GLogLevelFlags log_level, const GLogField *fields, gsize n_fields, gpointer user_data)
{
    ...
    return gdu_log_write (log_level, log_domain, log_message, fields, n_fields, user_data);
}
...
#if __has_feature(address_sanitizer) || defined(__SANITIZE_ADDRESS__) || defined(__SANITIZE_THREAD__)
    return;
#endif
...
gdu_log_init (void)
{
    ...
    const GDebugKey keys[] = { { "gc-friendly", 1 },
         ...
         const char *func, const char *message_format, ...)
{
    ...
    g_log_structured (domain, log_level, "CODE_FILE", file, "CODE_LINE", line, "CODE_FUNC", func, "MESSAGE", "%s",
                      str->str);
}
