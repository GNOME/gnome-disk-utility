...
static void
change_filesystem_label_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!gdu_block_set_fs_label_finish (self->drive_block, result, &error)) {
        gdu_utils_show_error (gdu_edit_filesystem_dialog_get_window (self), _("Error setting label"), error);
    }
...
on_change_button_clicked (GduEditFilesystemDialog *self)
{
    ...
    gdu_block_set_fs_label_async (self->drive_block, label, change_filesystem_label_cb, g_object_ref (self));
}
...
on_fs_label_row_changed_cb (GduEditFilesystemDialog *self)
{
    ...
    if (g_strcmp0 (gtk_editable_get_text (GTK_EDITABLE (self->fs_label_row)),
                   gdu_block_get_fs_label (self->drive_block))
        == 0) {
...
void
gdu_edit_filesystem_dialog_show (GtkWindow *parent_window, GduBlock *block)
{
    ...
    adw_banner_set_revealed (ADW_BANNER (self->warning_banner), gdu_block_needs_unmount (self->drive_block));
