...
on_done_clicked_cb (GduEncryptionOptionsDialog *self)
{
    ...
    if (self->crypttab_config != NULL && use_defaults) {
        if (!udisks_block_call_remove_configuration_item_sync (
                self->udisks_block, g_variant_new ("(s@a{sv})", "crypttab", self->crypttab_config),
            ...
            if (!g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
                gdu_utils_show_error (gdu_encryption_options_dialog_get_window (self),
                                      _("Error removing /etc/crypttab entry"), error);
        }
        return;
    }
    ...
    if (self->crypttab_config == NULL && new_crypttab != NULL) {
        if (!udisks_block_call_add_configuration_item_sync (self->udisks_block, new_crypttab,
                                                            g_variant_new ("a{sv}", NULL), /* options */
                                                            NULL,                          /* GCancellable */
                                                            &error)) {
            if (g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
                gdu_utils_show_error (gdu_encryption_options_dialog_get_window (self),
                                      _("Error adding /etc/crypttab entry"), error);
        }
        return;
    }
    ...
    if (old_crypttab != NULL && new_crypttab != NULL) {
        if (!udisks_block_call_update_configuration_item_sync (self->udisks_block, old_crypttab, new_crypttab,
                                                               g_variant_new ("a{sv}", NULL), /* options */
                                                               NULL,                          /* GCancellable */
                                                               &error)) {
            if (g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
                gdu_utils_show_error (gdu_encryption_options_dialog_get_window (self),
                                      _("Error updating /etc/crypttab entry"), error);
        }
        return;
    }
}
...
on_property_changed_cb (GduEncryptionOptionsDialog *self)
{
    ...
    gdu_options_update_check_option (GTK_WIDGET (self->options_entry), "x-udisks-auth",
                                     GTK_WIDGET (self->require_auth_switch), GTK_WIDGET (self->require_auth_switch),
                                     FALSE, FALSE);
    ...
    if (self->crypttab_config == NULL && !use_defaults)
        ...
        if (g_strcmp0 (new_name, old_name) != 0 || g_strcmp0 (new_options, old_options) != 0
...
void
gdu_encryption_options_dialog_show (GtkWindow *parent_window, UDisksClient *client, UDisksObject *object)
{
    ...
    self->crypttab_config =
        gdu_encryption_options_dialog_get_crypttab_from_config (udisks_block_get_configuration (self->udisks_block));
    ...
    if (self->crypttab_config != NULL) {
        ...
        if (passphrase_path && *passphrase_path && !g_str_has_prefix (passphrase_path, "/dev")) {
            if (!udisks_block_call_get_secret_configuration_sync (self->udisks_block,
                                                                  g_variant_new ("a{sv}", NULL), /* Options */
                                                                  &config, NULL,                 /* GCancellable */
                                                                  &error)) {
                gdu_utils_show_error (gdu_encryption_options_dialog_get_window (self),
                                      _("Error retrieving configuration data"), error);
                return;
            }
