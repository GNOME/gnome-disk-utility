...
static void on_client_changed (UDisksClient *client, gpointer user_data);

static void update (GduSdMonitor *monitor);

static void
udisks_client_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (monitor->client == NULL) {
        g_warning ("Error initializing udisks client: %s (%s, %d)", error->message, g_quark_to_string (error->domain),
                   error->code);
        g_clear_error (&error);
    } else {
        g_signal_connect (monitor->client, "changed", G_CALLBACK (on_client_changed), monitor);
        update (monitor);
    }
...
gdu_sd_monitor_init (GduSdMonitor *monitor)
{
    udisks_client_new (NULL, /* GCancellable* */
                       udisks_client_cb, g_object_ref (monitor));
}
...
typedef gboolean (*CheckProblemFunc) (GduSdMonitor *monitor, UDisksObject *object);

static void
update_problems (GduSdMonitor *monitor, GList **problem_list, CheckProblemFunc check_func)
{
    GList *want = NULL;
    GList *added = NULL;
    GList *removed = NULL;
    ...
    *problem_list = g_list_sort (*problem_list, ptr_compare);
    diff_sorted_lists (*problem_list, want, ptr_compare, &added, &removed);
...
static void
on_examine_action_clicked (NotifyNotification *notification, const gchar *action, gpointer user_data)
{
    GduSdMonitor *monitor = GDU_SD_MONITOR (user_data);
    ...
    if (g_strcmp0 (action, "examine-smart") == 0) {
        if (monitor->ata_smart_problems != NULL) {
            ...
            if (object != NULL) {
                ...
                if (drive != NULL) {
                    UDisksBlock *block =
                        udisks_client_get_block_for_drive (monitor->client, drive, TRUE); /* get_physical */
                    if (block != NULL) {
    ...
    app_info = g_app_info_create_from_commandline (command_line, NULL, /* application name */
                                                   G_APP_INFO_CREATE_SUPPORTS_STARTUP_NOTIFICATION, NULL);
    if (!g_app_info_launch (app_info, NULL, NULL, &error)) {
        g_warning ("Error launching gnome-disks: %s (%s, %d)", error->message, g_quark_to_string (error->domain),
                   error->code);
        g_clear_error (&error);
    }
...
static void
update_notification (GduSdMonitor *monitor, GList *problems, NotifyNotification **notification, const gchar *title,
                     const gchar *text, const gchar *icon_name, const gchar *action, const gchar *action_label)
{
    if (g_list_length (problems) > 0) {
        ...
        if (*notification == NULL) {
            *notification = notify_notification_new (title, text, icon_name);
            ...
            notify_notification_add_action (*notification, action, action_label,
                                            (NotifyActionCallback) on_examine_action_clicked, monitor, NULL);
...
static gboolean
check_for_ata_smart_problem (GduSdMonitor *monitor, UDisksObject *object)
{
    gboolean ret = FALSE;
...
update (GduSdMonitor *monitor)
{
    ...
    update_notification (monitor, monitor->ata_smart_problems, &monitor->ata_smart_notification,
                         /* Translators: This is used as the title of the SMART failure notification */
                         C_("notify-smart", "Hard Disk Problems Detected"),
                            /* Translators: This is used as the text of the SMART failure notification */
                            C_("notify-smart", "A hard disk is likely to fail soon."), "org.gnome.DiskUtility",
                               "examine-smart",
...
static void
on_client_changed (UDisksClient *client, gpointer user_data)
{
    GduSdMonitor *monitor = GDU_SD_MONITOR (user_data);
    update (monitor);
}
