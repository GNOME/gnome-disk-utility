...
static void
on_confirm_button_clicked_cb (GduEditPartitionDialog *self, GtkButton *button)
{
    ...
    if (g_strcmp0 (udisks_partition_get_type_ (self->udisks_partition), type) != 0) {
        if (!udisks_partition_call_set_type_sync (self->udisks_partition, type,
    ...
    if (g_strcmp0 (udisks_partition_get_name (self->udisks_partition), name) != 0) {
        if (!udisks_partition_call_set_name_sync (self->udisks_partition, name,
                                                  g_variant_new ("a{sv}", NULL), /* options */
                                                  NULL,                          /* GCancellable */
                                                  &error)) {
    ...
    if (udisks_partition_get_flags (self->udisks_partition) != flags) {
        if (!udisks_partition_call_set_flags_sync (self->udisks_partition, flags,
                                                   g_variant_new ("a{sv}", NULL), /* options */
                                                   NULL,                          /* GCancellable */
                                                   &error)) {
...
gdu_edit_partition_dialog_populate_types_row (GduEditPartitionDialog *self)
{
    ...
    infos = udisks_client_get_partition_type_infos (self->client, self->partition_table_type, NULL);
    for (l = infos; l != NULL; l = l->next) {
        ...
        s = udisks_client_get_partition_type_and_subtype_for_display (self->client, self->partition_table_type,
    ...
    g_signal_connect_swapped (G_OBJECT (self->type_row), "notify::selected-item", G_CALLBACK (on_property_changed_cb),
                              self);
}
...
gdu_edit_partition_dialog_init (GduEditPartitionDialog *self)
{
    ...
    adw_combo_row_set_model (ADW_COMBO_ROW (self->type_row), G_LIST_MODEL (self->model));
}

void
gdu_edit_partition_dialog_show (GtkWindow *parent_window, UDisksObject *object, UDisksClient *client)
{
    GduEditPartitionDialog *self;

    self = g_object_new (GDU_TYPE_EDIT_PARTITION_DIALOG, NULL);

    self->window = parent_window;
    self->client = client;
    self->udisks_object = g_object_ref (object);
    self->udisks_partition = udisks_object_get_partition (object);
    self->udisks_partition_table = udisks_client_get_partition_table (client, self->udisks_partition);
