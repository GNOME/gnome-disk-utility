...
{
    guint n;
    static const gchar *dirs[] = { "/",          "/boot",      "/home",    "/usr",           "/usr/local", "/var",
                                   "/var/crash", "/var/local", "/var/log", "/var/log/audit", "/var/mail",  "/var/run",
                                   "/var/tmp",   "/opt",       "/root",    "/tmp",           NULL };
...
gdu_mount_options_dialog_populate_device_combo_row (GduMountOptionsDialog *self)
{
    ...
    gtk_string_list_append (GTK_STRING_LIST (self->model), item);
    ...
    for (n = 0; symlinks != NULL && symlinks[n] != NULL; n++) {
        ...
        gtk_string_list_append (GTK_STRING_LIST (self->model), symlinks[n]);
    }

    item = udisks_block_get_id_uuid (self->block);
    if (item != NULL && strlen (item) > 0) {
        gtk_string_list_append (GTK_STRING_LIST (self->model), g_strdup_printf ("UUID=%s", item));
    }

    item = udisks_block_get_id_label (self->block);
    if (item != NULL && strlen (item) > 0) {
        gtk_string_list_append (GTK_STRING_LIST (self->model), g_strdup_printf ("LABEL=%s", item));
    }
...
static void
on_update_configuration_item_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!udisks_block_call_update_configuration_item_finish (block, res, &error)) {
        if (!g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
            gdu_utils_show_error (gdu_mount_options_dialog_get_window (self), _("Error updating /etc/fstab entry"),
                                                                                error);
...
static void
on_add_configuration_item_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduMountOptionsDialog *self = GDU_MOUNT_OPTIONS_DIALOG (user_data);
    UDisksBlock *block = UDISKS_BLOCK (source_object);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_block_call_add_configuration_item_finish (block, res, &error)) {
        if (!g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
            gdu_utils_show_error (gdu_mount_options_dialog_get_window (self), _("Error adding new /etc/fstab entry"),
                                                                                error);
        gtk_widget_set_visible (self->spinner, FALSE);
    } else {
        adw_dialog_close (ADW_DIALOG (self));
...
static void
on_remove_configuration_item_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduMountOptionsDialog *self = GDU_MOUNT_OPTIONS_DIALOG (user_data);
    UDisksBlock *block = UDISKS_BLOCK (source_object);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_block_call_remove_configuration_item_finish (block, res, &error)) {
        if (!g_error_matches (error, UDISKS_ERROR, UDISKS_ERROR_NOT_AUTHORIZED_DISMISSED))
            gdu_utils_show_error (gdu_mount_options_dialog_get_window (self), _("Error removing old /etc/fstab entry"),
                                                                                error);
        gtk_widget_set_visible (self->spinner, FALSE);
    } else {
        adw_dialog_close (ADW_DIALOG (self));
...
on_done_clicked_cb (GduMountOptionsDialog *self)
{
    ...
    if (self->orig_fstab_entry != NULL && !configured) {
        udisks_block_call_remove_configuration_item (self->block,
                                                     ...
                                                     NULL,                          /* GCancellable */
                                                     on_remove_configuration_item_cb, self);
        return;
    }
    ...
    if (old_fstab == NULL && new_fstab != NULL) {
        udisks_block_call_add_configuration_item (self->block, new_fstab, g_variant_new ("a{sv}", NULL), /* options */
                                                  NULL, /* GCancellable */
                                                  on_add_configuration_item_cb, self);
        return;
    }

    udisks_block_call_update_configuration_item (self->block, old_fstab, new_fstab,
                                                 g_variant_new ("a{sv}", NULL), /* options */
                                                 NULL,                          /* GCancellable */
                                                 on_update_configuration_item_cb, self);
}
...
static void
on_property_changed (GtkWidget *widget, GParamSpec *pspec, gpointer user_data)
{
    ...
    gdu_options_update_check_option (self->mount_options_row, "noauto", widget, self->startup_mount_switch, TRUE,
                                     FALSE);
    gdu_options_update_check_option (self->mount_options_row, "x-udisks-auth", widget, self->require_auth_switch, FALSE,
                                     FALSE);
    gdu_options_update_check_option (self->mount_options_row, "x-gvfs-show", widget, self->show_in_files_switch, FALSE,
                                     FALSE);
    ...
    can_ok = (has_fstab_entry && !use_manual_mount_options) || (!has_fstab_entry && use_manual_mount_options);
    ...
    if (use_manual_mount_options) {
        can_ok &= update_warning_css (self->mount_options_row) && update_warning_css (self->mount_point_row)
...
static void
fstab_on_device_combo_row_changed (GtkWidget *widget, GParamSpec *pspec, GduMountOptionsDialog *self)
{
...
gdu_mount_options_dialog_init (GduMountOptionsDialog *self)
{
    ...
    adw_combo_row_set_model (ADW_COMBO_ROW (self->device_combo_row), G_LIST_MODEL (self->model));
}

void
gdu_mount_options_dialog_show (GtkWindow *parent_window, UDisksObject *object, UDisksClient *client)
{
