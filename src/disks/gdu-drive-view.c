...
update_drive_view (GduDriveView *self)
{
    ...
    gtk_list_box_bind_model (self->drive_partitions_listbox, partitions, (GtkListBoxCreateWidgetFunc) gdu_block_row_new,
                             NULL, NULL);
...
on_copy_drive_model_clicked (GduDriveView *self)
{
    ...
    adw_toast_overlay_add_toast (self->drive_info_dialog_toast_overlay,
                                 adw_toast_new (_("Copied drive model to clipboard")));
}
...
on_copy_drive_serial_clicked (GduDriveView *self)
{
    ...
    adw_toast_overlay_add_toast (self->drive_info_dialog_toast_overlay,
                                 adw_toast_new (_("Copied drive serial to clipboard")));
}
...
create_disk_image_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_create_disk_image_dialog_show (drive_view_get_window (self), object, gdu_manager_get_client (manager));
}
...
smart_disk_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_ata_smart_dialog_show (drive_view_get_window (self), object, gdu_manager_get_client (manager));
}
...
drive_settings_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_disk_settings_dialog_show (drive_view_get_window (self), object, gdu_manager_get_client (manager));
}

static void
drive_view_standby_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!gdu_drive_standby_finish (self->drive, result, &error)) {
        gdu_utils_show_error (drive_view_get_window (self), _("An error occurred when trying to "
...
standby_drive_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_drive_standby_async (self->drive, NULL, drive_view_standby_cb, g_object_ref (self));
}

static void
drive_view_wakeup_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(GduDriveView) self = user_data;
    g_autoptr(GError) error = NULL;
...
wakeup_drive_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_drive_wakeup_async (self->drive, NULL, drive_view_wakeup_cb, g_object_ref (self));
}

static void
drive_view_detach_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(GduDriveView) self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    if (!gdu_drive_detach_finish (self->drive, result, &error)) {
        gdu_utils_show_error (drive_view_get_window (self), _("Error detaching loop device"), error);
    }
}
...
detach_loop_device_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    gdu_drive_detach_async (self->drive, drive_view_get_window (self), NULL, drive_view_detach_cb, g_object_ref (self));
}

static void
drive_view_eject_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(GduDriveView) self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    if (!gdu_drive_eject_finish (self->drive, result, &error)) {
        gdu_utils_show_error (drive_view_get_window (self), _("Error ejecting drive"), error);
    }
}

static void
eject_confirmation_response_cb (GObject *object, GAsyncResult *response, gpointer user_data)
{
    ...
    gdu_drive_eject_async (self->drive, drive_view_get_window (self), NULL, drive_view_eject_cb, g_object_ref (self));
}
...
eject_drive_clicked_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
    ...
    if (siblings == NULL) {
        gdu_drive_eject_async (self->drive, drive_view_get_window (self), NULL, drive_view_eject_cb,
                               g_object_ref (self));
        return;
    }
    ...
    affected_devices_widget = gdu_util_create_widget_from_objects (drive_view_get_client (), objects);
    ...
    gdu_utils_show_confirmation (drive_view_get_window (self), data, affected_devices_widget);
}

static void
drive_view_power_off_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(GduDriveView) self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    if (!gdu_drive_power_off_finish (self->drive, result, &error)) {
        gdu_utils_show_error (drive_view_get_window (self), _("Error powering off drive"), error);
    }
}

static void
poweroff_confirmation_response_cb (GObject *object, GAsyncResult *response, gpointer user_data)
{
    GduDriveView *self = GDU_DRIVE_VIEW (user_data);
    AdwAlertDialog *dialog = ADW_ALERT_DIALOG (object);
    ...
    gdu_drive_power_off_async (self->drive, drive_view_get_window (self), NULL, drive_view_power_off_cb,
                               g_object_ref (self));
}
...
static void
gdu_drive_view_set_mobile (GduDriveView *self, gboolean mobile)
{
...
static void
gdu_drive_view_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
gdu_drive_view_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    GduDriveView *self = GDU_DRIVE_VIEW (object);
...
void
gdu_drive_view_set_drive (GduDriveView *self, GduDrive *drive)
{
