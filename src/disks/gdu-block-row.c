...
gdu_block_row_update_information (GduBlockRow *self)
{
    ...
    adw_action_row_set_subtitle (ADW_ACTION_ROW (self->partition_type_row), partition != NULL ? partition : "—");
...
gdu_block_row_update_features (GduBlockRow *self)
{
    ...
    menu_item_attribute =
        g_menu_model_get_item_attribute_value (self->volume_actions_submenu, 0, "action", g_variant_type_new ("s"));
...
#define ENABLE(_action, _feature)                                                                                      \
    gtk_widget_action_set_enabled (GTK_WIDGET (self), (_action), (features & (_feature)) != 0)
...
format_partition_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_create_format_show (block_row_get_client (), block_row_get_window (self), object, FALSE, 0, 0);
}
...
edit_partition_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    object = gdu_block_get_object (self->block);
    g_assert (object != NULL);
    gdu_edit_partition_dialog_show (block_row_get_window (self), object, block_row_get_client ());
}
...
edit_filesystem_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_edit_filesystem_dialog_show (block_row_get_window (self), self->block);
}
...
create_partition_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_create_format_show (block_row_get_client (), block_row_get_window (self), object, TRUE,
                            gdu_block_get_offset (self->block), gdu_item_get_size (GDU_ITEM (self->block)));
}
...
unmount_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_utils_ensure_unused (block_row_get_client (), block_row_get_window (self), object, NULL, NULL, NULL);
}

static void
mount_complete_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduBlockRow *self = GDU_BLOCK_ROW (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (source_object);
    g_autoptr(GError) error = NULL;

    if (!udisks_filesystem_call_mount_finish (filesystem, NULL, /* out_mount_path */
                                              res, &error)) {
        gdu_utils_show_error (block_row_get_window (self), _("Error mounting filesystem"), error);
    }
}
...
mount_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    udisks_filesystem_call_mount (filesystem, g_variant_new ("a{sv}", NULL), /* options */
                                  NULL,                                      /* cancellable */
                                  (GAsyncReadyCallback) mount_complete_cb, self);
}
...
unlock_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_unlock_dialog_show (block_row_get_window (self), object);
}

static void
partition_delete_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GduBlockRow) self = user_data;
    UDisksPartition *partition = UDISKS_PARTITION (object);
    g_autoptr(GError) error = NULL;

    if (!udisks_partition_call_delete_finish (partition, res, &error)) {
        gdu_utils_show_error (block_row_get_window (self), _("Error deleting partition"), error);
    }
}

static void
delete_ensure_unused_cb (GObject *obj, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(GduBlockRow) self = user_data;
    UDisksObject *object;
    ...
    if (gdu_utils_ensure_unused_finish (block_row_get_client (), result, NULL)) {
        ...
        udisks_partition_call_delete (partition, g_variant_new ("a{sv}", NULL), /* options */
                                      NULL,                                     /* cancellable */
                                      partition_delete_cb, g_object_ref (self));
    }
}

static void
delete_response_cb (GObject *source_object, GAsyncResult *response, gpointer user_data)
{
    GduBlockRow *self = GDU_BLOCK_ROW (user_data);
    ...
    gdu_utils_ensure_unused (block_row_get_client (), block_row_get_window (self), object, delete_ensure_unused_cb,
                             NULL, /* GCancellable */
                             g_object_ref (self));
}
...
delete_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    affected_devices_widget = gdu_util_create_widget_from_objects (block_row_get_client (), objects);
    ...
    gdu_utils_show_confirmation (block_row_get_window (self), data, affected_devices_widget);
}
...
resize_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    if (g_strcmp0 (missing_util, "") > 0) {
        error = g_error_new (udisks_error_quark (), 0,
                             _("The utility “%s” is required to resize a %s partition."), missing_util, fs_type);

        gdu_utils_show_error (block_row_get_window (self), _("Error while resizing filesystem"), error);
    ...
    object = gdu_block_get_object (self->block);
    g_assert (object != NULL);
    gdu_resize_dialog_show (block_row_get_window (self), object, block_row_get_client ());
}

static void
fs_check_cb (GObject *obj, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!udisks_filesystem_call_check_finish (filesystem, &consistent, result, &error))
        return gdu_utils_show_error (block_row_get_window (self), _("Error while checking filesystem"), error);
...
static void
fs_check_unmount_cb (GObject *obj, GAsyncResult *result, gpointer user_data)
{
    GduBlockRow *self = user_data;
    UDisksObject *object;
    UDisksFilesystem *filesystem;
    ...
    object = gdu_block_get_object (self->block);
    filesystem = udisks_object_peek_filesystem (object);
    udisks_filesystem_call_check (filesystem, g_variant_new ("a{sv}", NULL), NULL, fs_check_cb, self);
}

static void
on_check_message_dialog_response (GObject *obj, GAsyncResult *response, gpointer user_data)
{
    GduBlockRow *self = user_data;
    ...
    gdu_utils_ensure_unused (block_row_get_client (), block_row_get_window (self), object, fs_check_unmount_cb, NULL,
                             g_object_ref (self));
}
...
check_fs_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_utils_show_confirmation (block_row_get_window (self), data, NULL);
}

static void
fs_repair_cb (GObject *obj, GAsyncResult *result, gpointer user_data)
{
    GduBlockRow *self = user_data;
    g_autoptr(GError) error = NULL;
    UDisksBlock *block;
    ...
    if (!udisks_filesystem_call_repair_finish (filesystem, &success, result, &error)) {
        gdu_utils_show_error (block_row_get_window (self), _("Error while repairing filesystem"), error);
        g_object_unref (self);
        return;
    }
    ...
    gdu_utils_show_message (success ? _("Repair successful")
                                    : _("Repair failed"),
                                        g_strdup_printf (success     ? _("Filesystem %s on %s has been repaired.")
...
static void
fs_repair_unmount_cb (GObject *obj, GAsyncResult *result, gpointer user_data)
{
    GduBlockRow *self = user_data;
    UDisksObject *object;
    UDisksFilesystem *filesystem;
    ...
    udisks_filesystem_call_repair (filesystem, g_variant_new ("a{sv}", NULL), NULL, fs_repair_cb, self);
}

static void
on_repair_message_dialog_response (GObject *source_object, GAsyncResult *response, gpointer user_data)
{
    GduBlockRow *self = GDU_BLOCK_ROW (user_data);
    AdwAlertDialog *dialog = ADW_ALERT_DIALOG (source_object);
    UDisksObject *object;
    ...
    gdu_utils_ensure_unused (block_row_get_client (), block_row_get_window (self), object, fs_repair_unmount_cb, NULL,
                             g_object_ref (self));
}
...
static void
fs_take_ownership_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    GduBlockRow *self = GDU_BLOCK_ROW (user_data);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_filesystem_call_take_ownership_finish (UDISKS_FILESYSTEM (object), result, &error)) {
        gdu_utils_show_error (block_row_get_window (self), _("Error while taking filesystem ownership"), error);
    }
}

void
on_recursive_switch_cb (GObject *source_object, GParamSpec *pspec, gpointer user_data)
{
    AdwAlertDialog *dialog = ADW_ALERT_DIALOG (source_object);
    gboolean active;
    ...
    adw_alert_dialog_set_default_response (dialog, active ? "cancel" : "confirm");
}
...
static void
on_take_ownership_dialog_response_cb (GObject *source_object, GAsyncResult *response, gpointer user_data)
{
    GduBlockRow *self;
    ...
    udisks_filesystem_call_take_ownership (filesystem, g_variant_builder_end (&options_builder), NULL,
                                           fs_take_ownership_cb, self);
}
...
take_ownership_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    adw_alert_dialog_format_body (dialog, _("Make your user and group the owner of ”%s”"),
    ...
    adw_alert_dialog_choose (dialog, block_row_get_window (self), NULL, /* GCancellable */
                             on_take_ownership_dialog_response_cb, data);
}
...
configure_fstab_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_mount_options_dialog_show (block_row_get_window (self), object, block_row_get_client ());
}
...
configure_crypttab_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_encryption_options_dialog_show (block_row_get_window (self), block_row_get_client (), object);
}
...
create_partition_image_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    object = gdu_block_get_object (self->block);
    g_assert (object != NULL);
    gdu_create_disk_image_dialog_show (block_row_get_window (self), object, block_row_get_client ());
}
...
restore_partition_image_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    gdu_rs_restore_disk_image_dialog_show (block_row_get_window (self), object_path, NULL);
}
...
benchmark_partition_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    object = gdu_block_get_object (self->block);
    g_assert (object != NULL);
    gdu_benchmark_dialog_show (block_row_get_window (self), object, block_row_get_client ());
}
...
gdu_block_row_new (GduBlock *block)
{
    ...
    g_signal_connect_object (self->block, "changed", G_CALLBACK (gdu_block_row_update), self, G_CONNECT_SWAPPED);
