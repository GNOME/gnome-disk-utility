...
gboolean
gdu_utils_has_configuration (UDisksBlock *block, const gchar *type, gboolean *out_has_passphrase)
{
    GVariantIter iter;
        ...
        if (g_strcmp0 (config_type, type) == 0) {
            if (g_strcmp0 (type, "crypttab") == 0) {
                ...
                if (g_variant_lookup (config_details, "passphrase-path", "^&ay", &passphrase_path)
                    && strlen (passphrase_path) > 0 && !g_str_has_prefix (passphrase_path, "/dev"))
                    has_passphrase = TRUE;
            }
            ret = TRUE;
...
gboolean
gdu_utils_has_userspace_mount_option (UDisksBlock *block, const gchar *option)
{
...
void
gdu_utils_configure_file_dialog_for_disk_images (GtkFileDialog *file_dialog, gboolean set_file_types,
...
static gboolean
has_option (GtkWidget *options_entry, const gchar *option, gboolean check_prefix, gchar **out_value)
{
    guint n;
    gboolean ret = FALSE;
    gchar **options;
...
void
gdu_options_update_check_option (GtkWidget *options_entry, const gchar *option, GtkWidget *widget,
                                 GtkWidget *check_button, gboolean negate, gboolean add_to_front)
{
    gboolean opts, ui;
...
gchar *
gdu_utils_format_duration_usec (guint64 usec, GduFormatDurationFlags flags)
{
    gchar *ret;
    guint64 years = 0;
    guint64 months = 0;
    ...
    if (years > 0) {
        /* Translators: Used for duration greater than one year. First %s is number of years, second %s is months, third
         * %s is days */
        ret = g_strdup_printf (C_("duration-year-to-inf", "%s, %s and %s"), years_str, months_str, days_str);
    } else if (months > 0) {
        /* Translators: Used for durations less than one year but greater than one month. First %s is number of months,
         * second %s is days */
...
guint
gdu_utils_count_primary_dos_partitions (UDisksClient *client, UDisksPartitionTable *table)
{
    GList *partitions, *l;
    guint ret = 0;
...
gboolean
gdu_utils_have_dos_extended (UDisksClient *client, UDisksPartitionTable *table)
{
    GList *partitions, *l;
    gboolean ret = FALSE;
...
gboolean
gdu_utils_is_inside_dos_extended (UDisksClient *client, UDisksPartitionTable *table, guint64 offset)
{
    GList *partitions, *l;
    gboolean ret = FALSE;
...
gdu_utils_show_message (const gchar *title, const gchar *message, GtkWidget *parent_window)
{
    ...
    adw_alert_dialog_add_responses (ADW_ALERT_DIALOG (dialog), "close", _("_Close"), NULL);
...
gdu_utils_show_error (GtkWindow *parent_window, const gchar *title, GError *error)
{
    ...
    message = g_strdup_printf ("%s (%s, %d)", fixed_up_error->message, g_quark_to_string (error->domain), error->code);
...
static GtkWidget *
widget_for_object (UDisksClient *client, UDisksObject *object)
{
    GtkWidget *row;
...
GtkWidget *
gdu_util_create_widget_from_objects (UDisksClient *client, GList *objects)
{
    GtkWidget *group;
...
void
gdu_utils_show_confirmation (GtkWidget *parent_window, ConfirmationDialogData *data, GtkWidget *extra_child)
{
    AdwAlertDialog *dialog;

    dialog = ADW_ALERT_DIALOG (adw_alert_dialog_new (data->message, data->description));

    adw_alert_dialog_add_responses (dialog, "cancel", _("_Cancel"), "confirm", data->response_verb, NULL);
    ...
    adw_alert_dialog_set_default_response (dialog,
                                           data->response_appearance == ADW_RESPONSE_SUGGESTED ? "confirm" : "cancel");

    if (extra_child)
        adw_alert_dialog_set_extra_child (dialog, extra_child);

    adw_alert_dialog_choose (dialog, parent_window, NULL, /* GCancellable */
...
gboolean
gdu_utils_can_resize (UDisksClient *client, const gchar *fstype, gboolean flush, ResizeFlags *mode_out,
                      gchar **missing_util_out)
{
    ...
    if (cache == NULL) {
        ...
        for (gsize i = 0; supported_fs[i] != NULL; i++) {
            ...
            if (udisks_manager_call_can_resize_sync (udisks_client_get_manager (client), supported_fs[i],
                                                     &out_available, NULL, NULL)) {
...
gboolean
gdu_utils_can_repair (UDisksClient *client, const gchar *fstype, gboolean flush, gchar **missing_util_out)
{
    static GHashTable *cache = NULL;
    const gchar *const *supported_fs;
    UtilCacheEntry *result;
    ...
    if (cache == NULL) {
        ...
        for (gsize i = 0; supported_fs[i] != NULL; i++) {
            ...
            if (udisks_manager_call_can_repair_sync (udisks_client_get_manager (client), supported_fs[i],
                                                     &out_available, NULL, NULL)) {
                UtilCacheEntry *entry;
...
gboolean
gdu_utils_can_format (UDisksClient *client, const gchar *fstype, gboolean flush, gchar **missing_util_out)
{
    static GHashTable *cache = NULL;
    const gchar *const *supported_fs;
    UtilCacheEntry *result;
    ...
    if (cache == NULL) {
        ...
        for (gsize i = 0; supported_fs[i] != NULL; i++) {
            ...
            if (udisks_manager_call_can_format_sync (udisks_client_get_manager (client), supported_fs[i],
                                                     &out_available, NULL, NULL)) {
                UtilCacheEntry *entry;
...
{
    if (g_strcmp0 (fstype, "ntfs") == 0 || g_strcmp0 (fstype, "vfat") == 0 || g_strcmp0 (fstype, "exfat") == 0) {
        return FALSE;
    }
    return TRUE;
}
...
gboolean
gdu_utils_can_check (UDisksClient *client, const gchar *fstype, gboolean flush, gchar **missing_util_out)
{
    static GHashTable *cache = NULL;
    const gchar *const *supported_fs;
    UtilCacheEntry *result;
    ...
    if (cache == NULL) {
        ...
        for (gsize i = 0; supported_fs[i] != NULL; i++) {
            ...
            if (udisks_manager_call_can_check_sync (udisks_client_get_manager (client), supported_fs[i], &out_available,
                                                    NULL, NULL)) {
                UtilCacheEntry *entry;
...
gboolean
_gtk_entry_buffer_truncate_bytes (GtkEntryBuffer *gtk_entry_buffer, guint max_bytes)
{
...
gboolean
gdu_util_is_same_size (GList *blocks, guint64 *out_min_size)
{
    gboolean ret = FALSE;
    ...
    if (max_size - min_size > min_size * 101LL / 100LL && max_size - min_size > 1048576)
        goto out;
...
GList *
gdu_utils_get_all_contained_objects (UDisksClient *client, UDisksObject *object)
{
...
static gboolean
gdu_utils_is_in_use_full (UDisksClient *client, UDisksObject *object, UDisksFilesystem **filesystem_to_unmount_out,
                          UDisksEncrypted **encrypted_to_lock_out, gboolean *last_out)
{
    ...
    if (filesystem_to_unmount_out != NULL) {
        *filesystem_to_unmount_out = (filesystem_to_unmount != NULL) ? g_object_ref (filesystem_to_unmount) : NULL;
    }
    if (encrypted_to_lock_out != NULL) {
        *encrypted_to_lock_out = (encrypted_to_lock != NULL) ? g_object_ref (encrypted_to_lock) : NULL;
    }
    if (last_out != NULL)
        *last_out = last;
...
gboolean
gdu_utils_is_in_use (UDisksClient *client, UDisksObject *object)
{
...
static void
unuse_data_complete (UnuseData *data, const gchar *error_message, GError *error)
{
    if (error != NULL) {
        gdu_utils_show_error (data->parent_window, error_message, error);
...
unuse_data_iterate (UnuseData *data)
{
    ...
    gdu_utils_is_in_use_full (data->client, object, &filesystem_to_unmount, &encrypted_to_lock, NULL);
        ...
        if (loop != NULL) {
            if (udisks_loop_get_autoclear (loop)) {
                gdu_utils_is_in_use_full (data->client,
                                          UDISKS_OBJECT (g_dbus_interface_get_object (G_DBUS_INTERFACE (loop))), NULL,
                                          NULL, &last);
                if (last) {
                    udisks_loop_call_set_autoclear (loop, FALSE, g_variant_new ("a{sv}", NULL), data->cancellable,
                                                    (GAsyncReadyCallback) unuse_set_autoclear_cb, data);
                    g_object_unref (loop);
                    g_clear_object (&encrypted_to_lock);
                    g_clear_object (&filesystem_to_unmount);
                    return;
    ...
    if (filesystem_to_unmount != NULL) {
        ...
        udisks_filesystem_call_unmount (filesystem_to_unmount, g_variant_new ("a{sv}", NULL), /* options */
                                        data->cancellable,                                    /* cancellable */
                                        (GAsyncReadyCallback) unuse_unmount_cb, data);
    } else if (encrypted_to_lock != NULL) {
        udisks_encrypted_call_lock (encrypted_to_lock, g_variant_new ("a{sv}", NULL), /* options */
                                    data->cancellable,                                /* cancellable */
                                    (GAsyncReadyCallback) unuse_lock_cb, data);
    } else {
...
void
gdu_utils_ensure_unused_list (UDisksClient *client, GtkWindow *parent_window, GList *objects,
                              GAsyncReadyCallback callback, GCancellable *cancellable, gpointer user_data)
{
    UnuseData *data;
...
gboolean
gdu_utils_ensure_unused_list_finish (UDisksClient *client, GAsyncResult *res, GError **error)
{
...
void
gdu_utils_ensure_unused (UDisksClient *client, GtkWindow *parent_window, UDisksObject *object,
                         GAsyncReadyCallback callback, GCancellable *cancellable, gpointer user_data)
{
    GList *objects;
...
gboolean
gdu_utils_ensure_unused_finish (UDisksClient *client, GAsyncResult *res, GError **error)
{
...
guint64
gdu_utils_calc_space_to_grow (UDisksClient *client, UDisksPartitionTable *table, UDisksPartition *partition)
{
    GList *partitions, *l;
...
guint64
gdu_utils_calc_space_to_shrink_extended (UDisksClient *client, UDisksPartitionTable *table, UDisksPartition *partition)
{
    GList *partitions, *l;
...
static gint
statvfs_for_block (UDisksClient *client, UDisksBlock *block, struct statvfs *statvfs_buf)
{
    gint64 ret = -1;
...
gint64
gdu_utils_get_unused_for_block (UDisksClient *client, UDisksBlock *block)
{
    gint64 ret = -1;
...
gint64
gdu_utils_get_total_size_for_block (UDisksClient *client, UDisksBlock *block)
{
    gint64 ret = -1;
    struct statvfs statvfs_buf;
