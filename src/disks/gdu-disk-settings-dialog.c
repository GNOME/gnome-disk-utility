...
compute_configuration (DialogData *data)
{
    ...
    if (data->orig_drive_configuration != NULL) {
        ...
        while (g_variant_iter_next (&iter, "{&sv}", &key, &value)) {
            if (g_strcmp0 (key, "ata-pm-standby") == 0 || g_strcmp0 (key, "ata-apm-level") == 0
                || g_strcmp0 (key, "ata-aam-level") == 0 || g_strcmp0 (key, "ata-write-cache-enabled") == 0) {
                /* handled by us, skip */
            } else {
...
static void
on_property_changed (GObject *object, GParamSpec *pspec, gpointer user_data)
{
...
update_standby_label (DialogData *data)
{
    ...
    if (value == 0) {
        ...
        s = gdu_utils_format_duration_usec (value * 5 * G_USEC_PER_SEC, GDU_FORMAT_DURATION_FLAGS_NONE);
    } else if (value < 252) {
        s = gdu_utils_format_duration_usec ((value - 240) * 30 * 60 * G_USEC_PER_SEC, GDU_FORMAT_DURATION_FLAGS_NONE);
    } else if (value == 252) {
        s = gdu_utils_format_duration_usec (21 * 60 * G_USEC_PER_SEC, GDU_FORMAT_DURATION_FLAGS_NONE);
    } else if (value == 253) {
...
static void
on_set_configuration_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    DialogData *data = user_data;
    GError *error = NULL;

    if (!udisks_drive_call_set_configuration_finish (UDISKS_DRIVE (source_object), res, &error)) {
        gdu_utils_show_error (data->window, _("Error setting configuration"), error);
        g_clear_error (&error);
        goto out;
    } else {
...
static void
on_ok_clicked (AdwAlertDialog *dialog, gpointer user_data)
{
    DialogData *data = user_data;

    udisks_drive_call_set_configuration (data->drive, compute_configuration (data), /* consumes floating */
                                         g_variant_new ("a{sv}", NULL),             /* options */
                                         NULL,                                      /* cancellable */
                                         on_set_configuration_cb, dialog_data_ref (data));
}
...
void
gdu_disk_settings_dialog_show (GtkWindow *window, UDisksObject *object, UDisksClient *client)
{
    DialogData *data;
    guint n;
    ...
    data->dialog =
        GTK_WIDGET (gdu_application_new_widget ((gpointer) g_application_get_default (), "gdu-disk-settings-dialog.ui",
                                                "disk-settings-dialog", &data->builder));
  ...
  g_signal_connect (data->aam_scale, "format-value", G_CALLBACK (on_aam_scale_format_value), data);
    ...
    if (data->orig_drive_configuration != NULL) {
        ...
        if (!write_cache_enabled_set) {
            ...
            gtk_combo_box_set_active (GTK_COMBO_BOX (data->write_cache_comboboxtext), write_cache_enabled ? 0 : 1);
            gtk_switch_set_active (GTK_SWITCH (data->write_cache_switch), TRUE);
        }
    }

    g_signal_connect (data->standby_switch, "notify::active", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->apm_switch, "notify::active", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->aam_switch, "notify::active", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->write_cache_switch, "notify::active", G_CALLBACK (on_property_changed), data);

    g_signal_connect (data->standby_adjustment, "notify::value", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->apm_adjustment, "notify::value", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->aam_adjustment, "notify::value", G_CALLBACK (on_property_changed), data);
    g_signal_connect (data->write_cache_comboboxtext, "notify::active", G_CALLBACK (on_property_changed), data);

    g_object_bind_property (data->standby_switch, "active", data->standby_widgets_box, "sensitive",
                            G_BINDING_SYNC_CREATE);

    g_object_bind_property (data->apm_switch, "active", data->apm_widgets_box, "sensitive", G_BINDING_SYNC_CREATE);

    g_object_bind_property (data->aam_switch, "active", data->aam_widgets_box, "sensitive", G_BINDING_SYNC_CREATE);

    g_object_bind_property (data->write_cache_switch, "active", data->write_cache_widgets_box, "sensitive",
                            G_BINDING_SYNC_CREATE);
    ...
    g_signal_connect (data->ok_button, "clicked", G_CALLBACK (on_ok_clicked), data);
...
gdu_disk_settings_dialog_should_show (UDisksObject *object)
{
    ...
    if ((udisks_drive_ata_get_pm_supported (ata) && !is_ssd) || udisks_drive_ata_get_apm_supported (ata)
