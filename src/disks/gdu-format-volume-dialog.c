...
gdu_format_volume_dialog_set_dialog_title (GduFormatVolumeDialog *self)
{
    ...
    child = gtk_stack_get_child_by_name (GTK_STACK (self->pages_stack), self->current);
...
gdu_format_volume_dialog_update_stack (GduFormatVolumeDialog *self)
{
    ...
    if (g_strcmp0 (self->current, PARTITION_PAGE) == 0) {
        ...
        self->prev = NULL;
        self->next = !gdu_create_partition_page_is_extended (self->partition_page) ? FORMAT_PAGE : NULL;
        return page;
    }
    ...
    if (g_strcmp0 (self->current, OTHER_PAGE) == 0) {
        ...
        self->next = gdu_create_other_page_is_encrypted (self->other_page) ? PASSWORD_PAGE : CONFIRM_PAGE;
        return page;
    }
    ...
    if (g_strcmp0 (self->current, CONFIRM_PAGE) == 0) {
        ...
        self->next = NULL;
        self->prev = gdu_create_filesystem_page_is_other (self->filesystem_page) ? OTHER_PAGE : FORMAT_PAGE;
...
static void
update_dialog (GtkWidget *widget, GParamSpec *child_property, GduFormatVolumeDialog *self)
{
    ...
    gtk_stack_set_visible_child_name (GTK_STACK (self->pages_stack), self->current);
...
format_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!udisks_block_call_format_finish (UDISKS_BLOCK (source_object), res, &error))
        gdu_utils_show_error (gdu_format_volume_dialog_get_window (self), _("Error formatting volume"), error);

    g_object_unref (self);
}

static void
ensure_unused_cb (UDisksClient *client, GAsyncResult *res, GduFormatVolumeDialog *self)
{
    ...
    udisks_block_call_format (self->udisks_block, fs_type, g_variant_builder_end (&options_builder),
                              NULL, /* GCancellable */
                              format_cb, self);
}

static void create_partition_cb (GObject *source_object, GAsyncResult *res, gpointer user_data);
...
on_forward_button_clicked_cb (GduFormatVolumeDialog *self, GtkWidget *widget)
{
    ...
    if (!self->add_partition) {
        ...
        gdu_utils_ensure_unused (self->udisks_client, gdu_format_volume_dialog_get_window (self), self->udisks_object,
    ...
    udisks_partition_table_call_create_partition (self->udisks_table, self->add_partition_offset, size,
                                                  partition_type, /* use default type */
                                                  "",             /* use blank partition name */
                                                  g_variant_builder_end (&options_builder), NULL, /* GCancellable */
                                                  create_partition_cb, g_object_ref (self));
}
...
create_partition_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!udisks_partition_table_call_create_partition_finish (UDISKS_PARTITION_TABLE (source_object),
        ...
        gdu_utils_show_error (gdu_format_volume_dialog_get_window (self), _("Error creating partition"), error);
        return;
    }
...
void
gdu_create_format_show (UDisksClient *client, GtkWindow *parent_window, UDisksObject *object,
                        gboolean add_partition, /* format vs add partition and format */
                        guint64 add_partition_offset, guint64 add_partition_maxsize)
{
    ...
    if (self->udisks_block == NULL && self->udisks_drive != NULL)
        self->udisks_block = udisks_client_get_block_for_drive (self->udisks_client, self->udisks_drive, FALSE);
    ...
    if (add_partition) {
        self->partition_page =
            gdu_create_partition_page_new (client, self->udisks_table, add_partition_maxsize, add_partition_offset);
        gtk_stack_add_titled (GTK_STACK (self->pages_stack), GTK_WIDGET (self->partition_page), PARTITION_PAGE,
                              _("Create Partition"));
    ...
    gtk_stack_add_titled (GTK_STACK (self->pages_stack), GTK_WIDGET (self->filesystem_page), FORMAT_PAGE,
                          _("Format Volume"));
    ...
    gtk_stack_add_titled (GTK_STACK (self->pages_stack), GTK_WIDGET (self->other_page), OTHER_PAGE, _("Custom Format"));
    ...
    gtk_stack_add_titled (GTK_STACK (self->pages_stack), GTK_WIDGET (self->password_page), PASSWORD_PAGE,
                          _("Set Password"));
    ...
    gtk_stack_add_titled (GTK_STACK (self->pages_stack), GTK_WIDGET (self->confirm_page), CONFIRM_PAGE,
                          _("Confirm Details"));
