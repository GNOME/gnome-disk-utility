...
static void
application_quit_response_cb (GObject *source_object, GAsyncResult *response, gpointer user_data)
{
...
static UDisksObject *
gdu_application_object_from_block_device (GduApplication *app, const gchar *block_device, gchar **error_message)
{
    struct stat statbuf;
    ...
    { "block-device", 0, 0, G_OPTION_ARG_STRING, NULL, N_("Select device"), "DEVICE" },
      { "format-device", 0, 0, G_OPTION_ARG_NONE, NULL, N_("Format selected device"), NULL },
        { "verbose", 'v', G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK, cmd_verbose_cb,
          N_("Show verbose logs, specify up to four times to increase log level"), NULL },
          { "xid", 0, 0, G_OPTION_ARG_INT, NULL, N_("Ignored, kept for compatibility"), "ID" },
            { "restore-disk-image", 0, 0, G_OPTION_ARG_FILENAME, NULL, N_("Restore disk image"), "FILE" }, { NULL } };
...
static gint
gdu_application_command_line (GApplication *_app, GApplicationCommandLine *command_line)
{
    ...
    if (opt_format && opt_block_device == NULL) {
        g_application_command_line_printerr (command_line,
                                             _("--format-device must be used together with --block-device\n"));
        goto out;
    }

    if (opt_format && opt_restore_disk_image != NULL) {
        g_application_command_line_printerr (command_line,
                                             _("--format-device must not be used together with --restore-disk-image"));
        goto out;
    }
    ...
    if (opt_restore_disk_image != NULL) {
        if (!g_file_test (opt_restore_disk_image, G_FILE_TEST_IS_REGULAR)) {
            g_application_command_line_printerr (command_line, "%s does not appear to be a regular file\n",
...
static void
new_disk_image_activated (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
static void
about_activated (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    GduApplication *app = GDU_APPLICATION (user_data);

    adw_show_about_dialog (GTK_WIDGET (app->window), "application-name",
                           _("Disks"), "application-icon", "org.gnome.DiskUtility", "developer-name",
                             _("The GNOME Project"), "version", PACKAGE_VERSION, "copyright",
                               _("© 2009 The GNOME Project\n© 2008-2013 Red Hat, Inc.\n© 2008-2013 David Zeuthen"),
                                 "website", "https://apps.gnome.org/DiskUtility/", "issue-url",
                                 "https://gitlab.gnome.org/GNOME/gnome-disk-utility/-/issues/", "license-type",
                                 GTK_LICENSE_GPL_2_0, "translator-credits", _("translator-credits"), NULL);
}
...
static GActionEntry app_entries[] = { { "new_disk_image", new_disk_image_activated, NULL, NULL, NULL },
...
gdu_application_startup (GApplication *_app)
{
    ...
    const gchar **it;
    const gchar *action_accels[] = { "win.go-back",
                                     "Escape",
                                     NULL,
                                     "win.open-drive-menu",
                                     "F9",
                                     NULL,
                                     "win.open-volume-menu",
                                     "<Shift>F9",
                                     NULL,
                                     "win.open-app-menu",
                                     "F10",
...
{
    return g_object_new (GDU_TYPE_APPLICATION, "application-id", "org.gnome.DiskUtility", "flags",
                         G_APPLICATION_HANDLES_COMMAND_LINE, "resource-base-path", "/org/gnome/DiskUtility", NULL);
}
...
GObject *
gdu_application_new_widget (GduApplication *application, const gchar *ui_file, const gchar *name,
