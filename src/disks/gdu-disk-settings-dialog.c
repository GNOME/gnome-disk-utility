...
G_DEFINE_FINAL_TYPE (GduDiskSettingsDialog, gdu_disk_settings_dialog, ADW_TYPE_DIALOG)
...
static GVariant *
gdu_disk_settings_dialog_get_new_configuration (GduDiskSettingsDialog *self, GVariant *orig_drive_configuration)
{
...
{
    g_autoptr(GVariant) new_drive_configuration = NULL;
    g_autoptr(GVariant) orig_drive_configuration = NULL;
    UDisksObject *object;
    gboolean changed;

    object = gdu_drive_get_object (self->drive);
    /* figure out if things has changed */
    orig_drive_configuration = udisks_drive_dup_configuration (udisks_object_peek_drive (object));
...
on_set_configuration_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!udisks_drive_call_set_configuration_finish (UDISKS_DRIVE (source_object), res, &error)) {
        gdu_utils_show_error (gdu_disk_settings_dialog_get_window (self), _("Error setting configuration"), error);
    }
...
on_done_clicked (GduDiskSettingsDialog *self)
{
    ...
    udisks_drive_call_set_configuration (drive,
                                         gdu_disk_settings_dialog_get_new_configuration (
                                             self, udisks_drive_get_configuration (drive)), /* consumes floating */
                                         g_variant_new ("a{sv}", NULL),                     /* options */
                                         NULL,                                              /* cancellable */
                                         on_set_configuration_cb, g_object_ref (self));
}
                               ...
                               GduDrive *drive)
{
    ...
    if (orig_drive_configuration != NULL) {
        ...
        gtk_adjustment_set_value (GTK_ADJUSTMENT (self->standby_time_adjustment),
                                  standby_value != -1 ? standby_value : 120);
        ...
        if (vendor_recommended_value >= 128 && vendor_recommended_value <= 254) {
            g_autofree gchar *description =
                g_strdup_printf (_("The vendor-recommended AAM value is %d."), vendor_recommended_value);
            gtk_accessible_update_property (GTK_ACCESSIBLE (self->aam_value_scale), GTK_ACCESSIBLE_PROPERTY_DESCRIPTION,
                                            description, -1);
            gtk_scale_add_mark (GTK_SCALE (self->aam_value_scale), vendor_recommended_value, GTK_POS_BOTTOM,
                                _("Vendor Recommended"));
        }

        adw_switch_row_set_active (ADW_SWITCH_ROW (self->override_write_cache_switch),
                                   write_cache_enabled_set ? TRUE : FALSE);
        adw_combo_row_set_selected (ADW_COMBO_ROW (self->write_cache_combo_row),
                                    write_cache_enabled_set ? (write_cache_enabled ? 0 : 1) : -1);
    }
