...
create_partition_update (GduCreatePartitionPage *self)
{
    ...
    if (g_strcmp0 (udisks_partition_table_get_type_ (self->table), "dos") == 0) {
        if (!gdu_utils_is_inside_dos_extended (self->client, self->table, self->offset)) {
            guint num_primary;
...
set_unit_num (GduCreatePartitionPage *self, gint unit_num)
{
    ...
    gtk_adjustment_configure (self->size_adjustment, value_units, 0.0, /* lower */
...
static void
gdu_create_partition_page_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
...
gdu_create_partition_page_new (UDisksClient *client, UDisksPartitionTable *table, guint64 max_size, guint64 offset)
{
    ...
    g_object_bind_property_full (self->size_adjustment, "value", self->free_following_adjustment, "value",
                                 G_BINDING_BIDIRECTIONAL, size_binding_func, size_binding_func, self, NULL);
