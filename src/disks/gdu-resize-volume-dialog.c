...
static gboolean
free_size_binding_func (GBinding *binding, const GValue *source_value, GValue *target_value, gpointer user_data)
{
...
static void
resize_get_usage_mount_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    ...
    gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self),
                          _("Error mounting filesystem to calculate minimum size"), error);
...
set_unit_num (GduResizeVolumeDialog *self, gint unit_num)
{
    ...
    gtk_adjustment_configure (self->size_adjustment, value_units, min_size_units,            /* lower */
                              max_size_units,                                                /* upper */
                              1,                                                             /* step increment */
                              100,                                                           /* page increment */
                              0.0);                                                          /* page_size */
    gtk_adjustment_configure (self->free_size_adjustment, max_size_units - value_units, 0.0, /* lower */
...
static void
part_resize_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    ...
    if (!udisks_partition_call_resize_finish (partition, res, &error)) {
        gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self), _("Error resizing partition"), error);
    }
...
static void
fs_resize_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    ...
    if (!udisks_filesystem_call_resize_finish (filesystem, res, &error)) {
        gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self), _("Error resizing filesystem"), error);
    }
...
static void
fs_repair_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_filesystem_call_repair_finish (filesystem, &success, res, &error) || !success) {
        gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self), _("Error repairing filesystem after resize"),
                                                                            error);
    }
...
static void
fs_resize_cb_offline_next_repair (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    udisks_filesystem_call_repair (self->filesystem, g_variant_new ("a{sv}", NULL), NULL, fs_repair_cb, self);
}

static void
fs_repair_cb_offline_next_grow (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_filesystem_call_repair_finish (filesystem, &success, res, &error) || !success) {
        gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self), _("Error repairing filesystem"), error);
        adw_dialog_close (ADW_DIALOG (self));
        return;
    }

    /* growing: next is to resize filesystem, then run repair again */
    udisks_filesystem_call_resize (self->filesystem, get_new_size (self), g_variant_new ("a{sv}", NULL), NULL,
                                   fs_resize_cb_offline_next_repair, self);
}

static void
ensure_unused_cb_offline_next_repair (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    ...
    udisks_filesystem_call_repair (self->filesystem, g_variant_new ("a{sv}", NULL), NULL,
                                   fs_repair_cb_offline_next_grow, self);
}

static void
part_resize_cb_offline_next_fs_unmount (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    ...
    gdu_utils_ensure_unused (self->client, gdu_resize_volume_dialog_get_window (self), self->object,
                             ensure_unused_cb_offline_next_repair, NULL, self);
}

static void
fs_repair_cb_next_part_resize (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    udisks_partition_call_resize (self->partition, get_new_size (self), g_variant_new ("a{sv}", NULL), NULL,
                                  (GAsyncReadyCallback) part_resize_cb, self);
}

static void
fs_resize_cb_offline_next_fs_repair (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    udisks_filesystem_call_repair (self->filesystem, g_variant_new ("a{sv}", NULL), NULL, fs_repair_cb_next_part_resize,
                                   self);
}

static void
fs_repair_cb_offline_next_shrink (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
...
static void
fs_resize_cb_online_next_part_resize (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
...
static void
part_resize_cb_online_next_fs_resize (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksPartition *partition = UDISKS_PARTITION (object);
    g_autoptr(GError) error = NULL;
...
static void
unmount_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
...
resize (GduResizeVolumeDialog *self)
{
    ...
    if ((self->support & ONLINE_SHRINK && shrinking) || (self->support & ONLINE_GROW && !shrinking)) {
        online_resize_no_repair (self);
        return;
    }

    if (shrinking) {
        gdu_utils_ensure_unused (self->client, gdu_resize_volume_dialog_get_window (self), self->object, unmount_cb,
                                 NULL, self);
        return;
    }
...
static void
mount_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    GduResizeVolumeDialog *self = GDU_RESIZE_VOLUME_DIALOG (user_data);
    UDisksFilesystem *filesystem = UDISKS_FILESYSTEM (object);
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_filesystem_call_mount_finish (filesystem, NULL, /* out_mount_path */
                                              res, &error)) {
        gdu_utils_show_error (gdu_resize_volume_dialog_get_window (self), _("Error mounting the filesystem"), error);
        adw_dialog_close (ADW_DIALOG (self));
        return;
    }
...
on_resize_clicked_cb (GduResizeVolumeDialog *self)
{
    ...
    if (g_strv_length ((gchar **) mount_points) == 0 && !offline_shrink) {
        ...
        udisks_filesystem_call_mount (self->filesystem, g_variant_new ("a{sv}", NULL), NULL, mount_cb, self);
        return;
    }
...
gdu_resize_volume_dialog_init (GduResizeVolumeDialog *self)
{
    ...
    self->cur_unit_num = -1;
    g_object_bind_property_full (self->size_adjustment, "value", self->free_size_adjustment, "value",
                                 G_BINDING_BIDIRECTIONAL, free_size_binding_func, free_size_binding_func, self, NULL);
}
...
gdu_resize_dialog_show (GtkWindow *parent_window, UDisksObject *object, UDisksClient *client)
{
    ...
    if (self->partition != NULL && udisks_partition_get_is_container (self->partition))
        self->min_size = gdu_utils_calc_space_to_shrink_extended (self->client, self->table, self->partition);
    ...
    if (self->filesystem != NULL) {
        ...
        available =
            gdu_utils_can_resize (self->client, udisks_block_get_id_type (self->block), FALSE, &self->support, NULL);
        g_assert (available);
    ...
    mount_points = self->filesystem != NULL ? udisks_filesystem_get_mount_points (self->filesystem) : NULL;
    ...
    if (self->filesystem != NULL && g_strv_length ((gchar **) mount_points) == 0) {
        ...
        udisks_filesystem_call_mount (self->filesystem, g_variant_new ("a{sv}", NULL), /* options */
                                      self->mount_cancellable, resize_get_usage_mount_cb, self);
    }
