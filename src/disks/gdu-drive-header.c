...
static void
gdu_drive_header_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
gdu_drive_header_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    GduDriveHeader *self = GDU_DRIVE_HEADER (object);
...
gdu_drive_header_class_init (GduDriveHeaderClass *klass)
{
    ...
    properties[PROP_ICON] = g_param_spec_object (
        "icon", NULL, NULL, G_TYPE_ICON, (G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS));
    properties[PROP_DRIVE_NAME] = g_param_spec_string (
        "drive-name", NULL, NULL, "", (G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS));
...
void
gdu_drive_header_set_icon (GduDriveHeader *self, GIcon *icon)
{
