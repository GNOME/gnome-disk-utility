...
static void
can_format_cb (UDisksManager *manager, GAsyncResult *res, gpointer user_data)
{
...
static void
gdu_create_other_page_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
...
static void
gdu_create_other_page_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    GduCreateOtherPage *self = GDU_CREATE_OTHER_PAGE (object);
...
gdu_create_other_page_new (UDisksClient *client)
{
    ...
    for (guint i = 0; i < N_OTHER_FS; i++) {
        ...
        gtk_actionable_set_action_name (GTK_ACTIONABLE (checkbox), "update_fs_type");
        gtk_actionable_set_action_target_value (GTK_ACTIONABLE (checkbox), g_variant_new_string (value->value_nick));
        ...
        udisks_manager_call_can_format (udisks_client_get_manager (self->client), value->value_nick, NULL,
                                        (GAsyncReadyCallback) can_format_cb, row);
    }
