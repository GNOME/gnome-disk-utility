...
#define return_if_progress(self, task)                                                                                 \
    do {                                                                                                               \
        if ((self)->in_progress) {                                                                                     \
            g_task_return_new_error ((task), G_IO_ERROR, G_IO_ERROR_PENDING, "A Process is already in progress");      \
            return;                                                                                                    \
        }                                                                                                              \
    } while (0)
...
lookup_cleartext_device_for_crypto_device (UDisksClient *client, const gchar *object_path)
{
    ...
    for (l = objects; l != NULL; l = l->next) {
        ...
        if (g_strcmp0 (udisks_block_get_crypto_backing_device (block), object_path) == 0) {
...
gdu_block_get_features (GduItem *item)
{
    ...
    features = drive_features & (GDU_FEATURE_CREATE_IMAGE | GDU_FEATURE_BENCHMARK | GDU_FEATURE_RESTORE_IMAGE);
    ...
    if (udisks_block_get_size (block) == 0
        && !(features & (GDU_FEATURE_CONFIGURE_FSTAB | GDU_FEATURE_CONFIGURE_CRYPTTAB))) {
        features |= GDU_FEATURE_CONFIGURE_FSTAB;
    }
    ...
    if (udisks_object_peek_filesystem (object) != NULL) {
        ...
        cleartext_device = lookup_cleartext_device_for_crypto_device (
            self->client, g_dbus_object_get_object_path (G_DBUS_OBJECT (object)));
    ...
    if (self->partition != NULL && g_strcmp0 (udisks_block_get_id_usage (block), "") == 0 && !read_only) {
        ...
        if (!read_only
            && (gdu_utils_can_repair (self->client, type, FALSE, &fs_repair_missing_utility)
...
GduBlock *
gdu_block_new (gpointer udisk_client, gpointer udisk_object, GduItem *parent)
{
    GduBlock *self;
...
GduBlock *
gdu_block_sized_new (gpointer udisk_client, guint64 start_offset, guint64 size, GduItem *parent)
{
    GduBlock *self;
...
gdu_block_get_size_str (GduBlock *self)
{
     ...
     */
    mount_points = gdu_block_get_mount_points (self);
    if (!mount_points || !mount_points[0] || gdu_block_is_extended (self))
...
gdu_block_needs_unmount (GduBlock *self)
{
    ...
    if (needs_unmount
        && (g_strcmp0 (fs_type, "ext2") == 0 || g_strcmp0 (fs_type, "ext3") == 0 || g_strcmp0 (fs_type, "ext4") == 0))
        needs_unmount = false;
...
static void
change_fs_label_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    GduBlock *self;
...
static void
unmount_fs_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    GduBlock *self;
    g_autoptr(GTask) task = user_data;
    GError *error = NULL;
    ...
    if (gdu_utils_ensure_unused_list_finish (self->client, result, &error)) {
        udisks_filesystem_call_set_label (self->file_system, label, g_variant_new ("a{sv}", NULL), /* options */
                                          NULL,                                                    /* cancellable */
                                          change_fs_label_cb, g_steal_pointer (&task));
    } else {
...
gdu_block_set_fs_label_async (GduBlock *self, const gchar *label, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (!self->file_system || !self->block) {
        g_debug ("Error setting fs label, file-system: %p, block: %p", self->file_system, self->block);

        g_task_return_new_error (task, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, "Device does not support setting label");
        return;
    }
    ...
    if (gdu_block_needs_unmount (self)) {
        ...
        gdu_utils_ensure_unused_list (self->client, NULL, objects, unmount_fs_cb, NULL, g_steal_pointer (&task));
    } else {
        udisks_filesystem_call_set_label (self->file_system, label, g_variant_new ("a{sv}", NULL), /* options */
                                          NULL,                                                    /* cancellable */
                                          change_fs_label_cb, g_steal_pointer (&task));
...
gboolean
gdu_block_set_fs_label_finish (GduBlock *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (GDU_IS_BLOCK (self), FALSE);
