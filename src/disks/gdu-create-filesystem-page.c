...
update_text_entry (GduCreateFilesystemPage *self)
{
    ...
    gtk_editable_set_text (GTK_EDITABLE (self->name_entry), g_string_free_and_steal (s));
}

static void
on_fs_name_changed (GduCreateFilesystemPage *self, GtkWidget *widget)
{
...
static void
gdu_create_filesystem_page_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
...
static void
gdu_create_filesystem_page_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    GduCreateFilesystemPage *self = GDU_CREATE_FILESYSTEM_PAGE (object);
...
gdu_create_filesystem_page_new (UDisksClient *client, UDisksDrive *drive)
{
    ...
    if (drive != NULL && udisks_drive_get_removable (drive)) {
        ...
        if (gdu_utils_can_format (client, "vfat", FALSE, NULL)
            && (gdu_utils_is_flash (drive) || udisks_drive_get_size (drive) < 20UL * 1000UL * 1000UL * 1000UL
