...
G_DEFINE_TYPE_WITH_CODE (GduXzDecompressor, gdu_xz_decompressor, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (G_TYPE_CONVERTER, gdu_xz_decompressor_iface_init))
...
init_lzma (GduXzDecompressor *decompressor)
{
    ...
    ret = lzma_stream_decoder (&decompressor->stream, UINT64_MAX, /* memlimit */
                               0);                                /* flags */
    if (ret != LZMA_OK)
...
gdu_xz_decompressor_new (void)
{
    ...
    decompressor = g_object_new (GDU_TYPE_XZ_DECOMPRESSOR, NULL);
...
gdu_xz_decompressor_convert (GConverter *converter, const void *inbuf, gsize inbuf_size, void *outbuf,
                             gsize outbuf_size, GConverterFlags flags, gsize *bytes_read, gsize *bytes_written,
                             GError **error)
{
    GduXzDecompressor *decompressor = GDU_XZ_DECOMPRESSOR (converter);
    lzma_ret res;
    ...
    if (res == LZMA_FORMAT_ERROR) {
        g_set_error (error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Internal error"));
        return G_CONVERTER_ERROR;
    }
...
gdu_xz_decompressor_get_uncompressed_size (GFile *compressed_file)
{
    ...
    if (mapped_file == NULL) {
        g_warning ("Error mapping file '%s': %s", path, error->message);
        g_clear_error (&error);
        goto out;
    }
    ...
    res = lzma_index_buffer_decode (&index_object, &memlimit, NULL /* allocator */, index, &bufpos, footer - index);
    if (res != LZMA_OK)
        goto out;
