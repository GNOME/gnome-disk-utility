...
on_dialog_entry_changed (GduChangePassphraseDialog *self)
{
    ...
    if (strlen (curr_pass) > 0 && strlen (new_pass) > 0 && g_strcmp0 (new_pass, confirm_pass) == 0
...
static void
update_configuration_item_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!udisks_block_call_update_configuration_item_finish (self->udisks_block, res, &error))
        gdu_utils_show_error (gdu_change_passphrase_dialog_get_window (self), _("Error updating /etc/crypttab"), error);

    adw_dialog_close (ADW_DIALOG (self));
}

static void
change_passphrase_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduChangePassphraseDialog *self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    if (!udisks_encrypted_call_change_passphrase_finish (self->udisks_encrypted, res, &error))
        gdu_utils_show_error (gdu_change_passphrase_dialog_get_window (self), _("Error changing passphrase"), error);
    ...
    while (g_variant_iter_next (&iter, "{sv}", &key, &value)) {
        if (g_strcmp0 (key, "passphrase-contents") == 0) {
            g_variant_builder_add (
                &builder, "{sv}", "passphrase-contents",
    ...
    udisks_block_call_update_configuration_item (
        self->udisks_block, g_variant_new ("(s@a{sv})", "crypttab", self->crypttab_details),
        g_variant_new ("(sa{sv})", "crypttab", &builder), g_variant_new ("a{sv}", NULL), /* options */
        NULL,                                                                            /* cancellable */
        update_configuration_item_cb, self);
}
...
on_change_passphrase_clicked (GduChangePassphraseDialog *self)
{
    udisks_encrypted_call_change_passphrase (
        self->udisks_encrypted, gtk_editable_get_text (GTK_EDITABLE (self->curr_pass_row)),
        gtk_editable_get_text (GTK_EDITABLE (self->new_pass_row)), g_variant_new ("a{sv}", NULL), /* options */
        NULL,                                                                                     /* GCancellable */
        change_passphrase_cb, self);
}
...
static void
on_get_secret_configuration_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    GduChangePassphraseDialog *self = user_data;
    GVariantIter iter;
    const gchar *type;
    ...
    gdu_utils_show_error (gdu_change_passphrase_dialog_get_window (self),
                          _("/etc/crypttab configuration data is malformed"), NULL);
...
void
gdu_change_passphrase_dialog_show (GtkWindow *window, UDisksObject *object, UDisksClient *client)
{
    ...
    if (self->has_passphrase_in_conf) {
        ...
        udisks_block_call_get_secret_configuration (self->udisks_block, g_variant_new ("a{sv}", NULL), /* options */
                                                    NULL,                                              /* cancellable */
                                                    on_get_secret_configuration_cb, self);
        return;
    }
